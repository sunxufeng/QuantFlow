...
**里程碑进度**：M1 最小闭环 ✅ → M2 数据/回测/执行引擎 ✅ → M3 节点库与前端编辑器 ✅ → M4 业务能力（用户/RBAC/项目/日志/监控/Docker）✅ → M5 测试与发布 ✅ → **V1.3 核心引擎补完 ✅**（期货回测 + 示例工作流模板库，版本 1.3.0）→ **V1.4 LLM 配置页 ✅**（自定义大模型持久化 + 连通性测试，版本 1.4.0）→ **V1.5 自包含增强包 ✅**（绩效归因增强 + 因子库扩展 + 工作流版本管理 + 移动端适配，版本 1.5.0）→ **V1.6 报告与因子可视化 ✅**（回测报告中心页 + 因子分析图表补全 + 白屏修复，版本 1.6.0）→ **V1.7 登录闭环 + 券商凭证配置 ✅**（前端登录门禁 + 后端读接口鉴权 + 券商凭证可配置页，版本 1.7.0）
...
| 回测报告中心（V1.6） | 前端「回测报告」页：横向对比所有历史回测（`GET /backtest/reports` 返回的 `summaries`：策略/标的/区间/总收益/年化/夏普/最大回撤/胜率），一键查看净值曲线详情与基准对比，运行回测后端自动留痕 |
| 因子分析可视化（V1.6） | 因子库「分析报告」面板校正指标字段映射（IC 均值 / IR / 正向占比 / 多空收益），新增 IC 序列折线图与 IC 衰减柱状图（SVG），直观呈现因子有效性与衰减速度 |
| 前端白屏修复（V1.6） | 修复 `Canvas` 组件中 `applyWorkflow`/`specOf` 因 `const` 暂存死区（TDZ）被 `useEffect` 依赖数组提前引用导致的整树崩溃白屏；jsdom 渲染校验 `#root` 非空、零运行时错误 |
| 登录闭环（V1.7） | 前端未登录只渲染全屏登录/注册页（不暴露任何业务内容）；后端为 `backtest`/`workflows`/`runs`/`execution`/`market` 读接口加 `Depends(get_current_user)`（含运行实例 WebSocket 经 `?token=` 鉴权），`/auth` 与 `/api/health` 保持公开；`api.js` 全局捕获 401 自动回到登录页 |
| 券商凭证配置页（V1.7） | 顶部导航「券商设置」（admin 可见）图形化配置券商类型 / API Key / Secret / Base URL / 账户 ID，持久化到 `app_settings` 并脱敏（GET 返回 `****` 尾 4 位）；连通性测试不依赖真实凭证（`none`/`simulated` 离线判定，真实券商做轻量可达性探测）；`LiveExecutionGateway` 改读持久化配置，凭证缺失仍 `GatewayNotConfigured`，凭证申请中不影响后续开发 |
| 测试 | 375 个 pytest 用例（含券商凭证配置 6 例）+ 前端生产构建，GitHub Actions CI（backend 单测 / frontend 构建 / Docker 构建） |
...
```bash
cd backend && .venv/bin/python -m pytest tests/ -q   # 375 个用例
```
