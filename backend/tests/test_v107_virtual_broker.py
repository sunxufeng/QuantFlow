...
虚拟账本为进程级、按账户键隔离的集合，每个用例前后清空以保证测试隔离。
"""
...
from app.core.broker.virtual import VirtualBrokerConnector, _VIRTUAL_BOOKS
...
def _reset_books():
    _VIRTUAL_BOOKS.clear()


@pytest.fixture(autouse=True)
def _reset_virtual_books():
    _reset_books()
    yield
    _reset_books()
...
def test_place_live_order_via_virtual(monkeypatch):
    ...
    positions = engine.live_positions("u1")
    assert any(p["symbol"] == "000001.SZ" and p["quantity"] == 200 for p in positions)
    # 账户快照含盈亏
    acc = engine.live_account(account_key="u1")
...
def test_virtual_accounts_are_isolated_by_user():
    client.put("/api/settings/broker", json={"broker": "virtual"})
    other = TestClient(app)
    other.post("/api/auth/register", json={"username": "v107_other", "password": "secret123"})
    token = other.post(
        "/api/auth/login", json={"username": "v107_other", "password": "secret123"}
    ).json()["token"]
    ...
    assert client.get("/api/trading/live/positions").json()
    assert other.get("/api/trading/live/positions").json() == []
    assert other.get("/api/trading/live/account").json()["equity"] == 1_000_000.0
    client.put("/api/settings/broker", json={"broker": "none"})


def test_api_live_account_requires_broker_config():
    client.put("/api/settings/broker", json={"broker": "none"})
    r = client.get("/api/trading/live/account")
