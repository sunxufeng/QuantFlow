...
def test_bars_to_table():
    ...
    assert table.columns == ["symbol", "date", "open", "high", "low", "close", "volume", "amount"]
    assert len(table) == 2
...
def test_local_source_fetch_range():
    src = LocalDataSource()
    bars = src.fetch_daily("600519.SH", "2024-01-02", "2024-01-05")
    assert len(bars) == 4
...
def test_cache_key_deterministic():
    k1 = cache_key("local", "600519.SH", "2024-01-01", "2024-02-01", "daily")
    k2 = cache_key("local", "600519.SH", "2024-01-01", "2024-02-01", "daily")
    assert k1 == k2
    assert len(k1) == 24
...
def test_service_primary_local():
    ...
    bars = svc.bars("600519.SH")
    assert len(bars) == 20
    assert bars[0].close > 0


def test_service_empty_primary_falls_back_to_local():
    svc = MarketService(primary=_EmptySource(), cache=MemoryCache())
    bars = svc.bars("600519.SH", start="2024-01-02", end="2024-01-05")
    assert len(bars) == 4  # 降级到 local


def test_service_failing_primary_falls_back():
    svc = MarketService(primary=_FailSource(), cache=MemoryCache())
    bars = svc.bars("000001.SZ", start="2024-01-02", end="2024-01-04")
    assert len(bars) == 3
